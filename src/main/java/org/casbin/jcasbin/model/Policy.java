...
public class Policy {
    ...
    public boolean updatePolicy(String sec, String ptype, List<String> oldRule, List<String> newRule) {
        ...
        Assertion assertion = model.get(sec).get(ptype);
        int index = assertion.policyIndex.get(oldRule.toString());
        assertion.policy.set(index, newRule);
        assertion.policyIndex.remove(oldRule.toString());
        assertion.policyIndex.put(newRule.toString(), assertion.policy.size());
        return true;
    }
    ...
    public boolean removePolicy(String sec, String ptype, List<String> rule) {
        Assertion assertion = model.get(sec).get(ptype);
        if (assertion.policyIndex.containsKey(rule.toString())) {
            int index = assertion.policyIndex.get(rule.toString());
            assertion.policy.remove(index);
            assertion.policyIndex.remove(rule.toString());
            for (int i = index; i < assertion.policy.size(); ++i) {
                assertion.policyIndex.put(assertion.policy.get(i).toString(), i);
            }
            return true;
        }
